...
class PeerConnection:
    ...
    def __init__(self, queue: Queue, info_hash,
                 peer_id, piece_manager, activepeers, on_block_cb=None):
        """
    ...
    async def _start(self):
        while 'stopped' not in self.my_state:
            ...
            try:
                ...
                self.activepeers[(ip, port)] = "connecting"
                self.reader, self.writer = await asyncio.wait_for(connect, timeout=5)
                self.activepeers[(ip, port)] = "handshake "
                status += 1
                ...
                self.activepeers[(ip, port)] = "interested"
                status += 1
                # print("successful")
                ...
                # print("trying interesed")
                await self._send_interested()
                self.activepeers[(ip, port)] = "   alive  "
                status += 1
                # print("successful")
                ...
                m = f"Connection to {ip} was failed by "
                log = "timeout"
                logging.error(m + log)
                await self._start()
            except Exception:
                # self.piece_manager.torrentClient.activpeers[[ip, port]] = None
                self.activepeers.pop((ip, port))
                m = f"Connection to {ip} was failed by "
                if status == 0:
                    log = "open connection"
                elif status == 1:
            ...
            try:
                async for message in PeerStreamIterator(self.reader, buffer):
                    ...
                    if type(message) is BitField:
                        ...
                        # print(f"Block ({message.index}:{message.begin // (2 ** 14)}) was arrived.")
                        self.on_block_cb(
