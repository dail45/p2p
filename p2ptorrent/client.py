...
from p2ptorrent.protocol import PeerConnection, REQUEST_SIZE
from p2ptorrent.tracker import Tracker
...
class TorrentClient:
    ...
    """
    def __init__(self, torrent, server=None):
        ...
        self.piece_manager = PieceManager(torrent, server)
        self.abort = False
        self.activepeers = {}
    ...
    async def start(self):
        ...
        while True:
            ...
            if (not previous) or (previous + interval < current):
                if (not th or not th.is_alive()) and not self.tracker.result:
                    th = threading.Thread(target=self.tracker.connect, args=(previous if previous else False,
                                                                             self.piece_manager.bytes_uploaded,
                                                                             self.piece_manager.bytes_downloaded))
                    th.start()
                    previous = current
            elif self.tracker.result:
                previous = current
                self._empty_queue()
                for peer in self.tracker.result:
                    self.available_peers.put_nowait((peer['ip'], peer['port']))
                self.tracker.result = []
            else:
                if len(self.activepeers) > 0:
                    maxl = len("255.255.255.255:99999")
                    sepint = 1
                    sep = "+---" + ("+" + "-" * maxl + "+" + "-" * 10) * sepint + "+"
                    setx = []
                    res = "\n" + sep + "\n"
                    for k, v in self.activepeers.items():
                        setx.append([k, v])
                    setx = sorted(setx, key=lambda x: x[1])
                    counter = 1
                    res = "\n" + sep + "\n"
                    for i in range(len(setx)):
                        line = "|" + " " * (3 - len(str(counter))) + str(counter) + "|"
                        counter += 1
                        para = setx[i]
                        id = ":".join(map(str, para[0]))
                        status = para[1]
                        lid = len(id)
                        delta = maxl - lid
                        if delta > 0:
                            if delta % 2 == 0:
                                templine = " " * (delta // 2) + id + " " * (delta // 2)
                            else:
                                templine = " " * int(delta / 2) + id + " " * (int(delta / 2) + 1)
                            line += templine
                            line += "|"
                        else:
                            line += id
...
class PieceManager:
    ...
    this implementation.
    """
    def __init__(self, torrent, server):
        ...
        self.server = server
        if self.server:
            pieces = self.server.get_pieces()
            self._fix_pieces(pieces)
        #self.fd = os.open(self.torrent.output_file,  os.O_RDWR | os.O_CREAT)

    def _initiate_pieces(self) -> [Piece]:
        # TODO Поддержка мультифайловых торрентов(неправильное определение фрагментов(pieces) для файлов)
        """
    ...
    def _fix_pieces(self, pieces):
        for piece in self.missing_pieces:
            if piece.index in pieces:
    ...
    def _get_rarest_piece(self, peer_id):
        ...
        rarest_piece = self.missing_pieces[0]
    ...
    def _write(self, piece):
        ...
        # print("-----")
        # print(piece)
        # print(f"index: {piece.index}")
        # print(f"lb: {len(piece.blocks)}")
        # print(f"hash: {piece.hash}")

        # pos = piece.index * self.torrent.piece_length
        data = piece.data
        res = [piece.index, data]
        self.server.writeData(res)
